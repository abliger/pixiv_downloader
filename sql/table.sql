...
create table follow_user(id integer primary key autoincrement,user_name text not null,user_id text not null,user_comment text,finish numeric);

create table log(id integer primary key autoincrement,message txt,path text,stack text,create_date text);

create table reDownloadImg(id integer primary key autoincrement,url text not null,img_id text not null,content text not null,finish numeric);