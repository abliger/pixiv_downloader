...
class Pixiv {
    ...
    async getFollowPageInfoAll(userId: string) {
        let user: User[] = []
        let { users, total } = await this.getFollowPageInfo(userId)
        user.push(...users)
        ...
        let bigUsers = await Promise.all(getFollowingUserIter(this))
        bigUsers.forEach(v => {
            user.push(...v.users)
        })
        return user
    ...
    async download(img_info: PhoneImgDownloadInfo) {
        let contentStr = JSON.stringify(img_info.illust_details)
        let date = new Date(img_info.illust_details.reupload_timestamp ? img_info.illust_details.reupload_timestamp * 1000 : img_info.illust_details.upload_timestamp * 1000)
        if (Number(img_info.illust_details.page_count) === 1) {
            let count = selectImgByUrl.get(img_info.illust_details.url_big) as { count: number }
            let fileName = this.getfileName(img_info.author_details.user_name, img_info.illust_details.title, img_info.illust_details.id, date)
            let flag = await Bun.file(fileName).exists()
            if (flag) {
                if (count.count === 0) {
        ...
        let imgInfos = img_info.illust_details.manga_a
        let current = 0
        while (current < imgInfos.length) {
            await Promise.all(imgInfos.slice(current, current + 4).map(async (url) => {
                ...
                let count = selectImgByUrl.get(url.url_big) as { count: number }
                let fileName = this.getFileNameDoc(img_info.author_details.user_name, img_info.illust_details.title, img_info.illust_details.id, date, url.page)
                let flag = await Bun.file(fileName).exists()
                if (flag) {
                    if (count.count === 0) {
