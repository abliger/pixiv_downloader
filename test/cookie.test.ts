...
describe.only("cookie.ts test", () => {
    test("cookie", async done => {
        let changeRow = db.prepare("update account set cookies=null,user_id=null").run()
        let cookie = (await import("../src/cookie")).default
        ...
        let phpsessid = cookie.cookieArray?.find(v => {
